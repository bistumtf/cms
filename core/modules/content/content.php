...
class content extends admin {
		...
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					...
					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a catid='\$catid' href='?m=content&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right' onclick='open_list(this)'>\$catname</a></span>";
	...
	public function public_category() {
		...
		if(!$catid) showmessage(L('missing_part_parameters'),'blank');
		$page = intval($_GET['page']);
		$page = max($page,1);
		...
		$template = $template ? $template : $CAT['setting']['category_template'];
