...
class create_html extends admin {
	...
	*/
	...
	public function category_index(){
		$catid=$_GET['catid'];
		$urlrules=getcache("urlrules","commons")[2];
		$categorys=getcache("category_content_$this->siteid","commons");

		$categoryInfo=$categorys[$catid];
		$catdir=$categoryInfo['catdir'];
		$url_mapping= pc_base::load_config('url','url_mapping');
		extract($url_mapping);

		$url=str_replace('{$catdir}',$catdir,$urlrules);
		$this->html = pc_base::load_app_class('html');
		$setting = string2array($categoryInfo['setting']);
		ob_start();
		include template("content",$setting['category_template']);
		$this->html->show_category($url);
		$referer = isset($referer) ? urlencode($referer) : '';
		showmessage(L('create_update_success'),'?m=content&c=content&a=init&catid='.$catid,200);
	}
