...
<span>|</span><a href="?m=content&c=create_html&a=category_index&catid=<?php echo $catid;?>&dosubmit=1&pc_hash=<?php echo $pc_hash;?>&referer=<?php echo urlencode($_SERVER['QUERY_STRING']);?>"><em><?php echo L('update_category_index');?></em></a>
</div>
...
<tbody>
	...
	?>
		...
		<td>
		<?php
			echo '<a href="javascript:;" onclick=\'window.open("?m=content&c=content&a=public_preview&steps='.$steps.'&catid='.$catid.'&id='.$r['id'].'","manage")\'>[预览]&nbsp&nbsp&nbsp&nbsp</a>';
		if($status==99) {
			if($r['islink']) {
